...
call :pick_bin mad_batch_eval.exe BIN
if not defined BIN (
  echo [MAD] batch binary not found.
...
echo [MAD] Running batch evaluation
echo [MAD] Using %BIN%
"%BIN%"
set "RET=%ERRORLEVEL%"
if not "%RET%"=="0" (
  echo [MAD] batch evaluation failed with exit code %RET%
  if /i not "%MAD_NO_PAUSE%"=="1" pause
  exit /b %RET%
)
if /i not "%MAD_NO_PAUSE%"=="1" pause
