...
call :pick_bin mad_visualizer.exe BIN
if not defined BIN (
  echo [MAD] visualizer binary not found.
...
echo [MAD] Running visualizer %SCENARIO%
echo [MAD] Using %BIN%
pushd "%ROOT_DIR%" >nul
"%BIN%" "%SCENARIO%"
set "RET=%ERRORLEVEL%"
popd >nul
if not "%RET%"=="0" (
  echo [MAD] visualizer failed with exit code %RET%
  if /i not "%MAD_NO_PAUSE%"=="1" pause
  exit /b %RET%
)
if /i not "%MAD_NO_PAUSE%"=="1" pause
